...
export const ChatWindow: FC<ChatWindowProps> = ({
  ...
  return (
    <motion.div
      initial={{ transform: "translateX(600px)" }}
      animate={{ transform: "translateX(0px)" }}
      exit={{ transform: "translateX(600px)" }}
      transition={{ type: "spring" }}
      className="fixed bottom-0 right-0 z-50 flex h-screen min-h-96 w-screen overflow-clip bg-white text-zinc-600 shadow-md ring-1 ring-zinc-100 dark:bg-zinc-900 dark:text-zinc-400 dark:ring-zinc-300/20 xl:bottom-7 xl:right-7 xl:h-[calc(100vh/2)] xl:w-[calc(100vw/3)] xl:min-w-96 xl:max-w-[28rem] xl:rounded-xl"
    >
      <div className="relative flex flex-1 flex-col items-stretch justify-between">
        ...
        <button
          onClick={toggleChat}
          className="absolute left-4 top-4 flex h-6 w-6 items-center justify-center rounded-full bg-zinc-50 p-1 shadow-sm ring-1 ring-zinc-100 dark:bg-black dark:ring-zinc-300/20 xl:left-2 xl:top-2"
        >
          <CloseIcon />
        </button>
      </div>
